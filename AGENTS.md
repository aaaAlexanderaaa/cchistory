...
- The currently registered source adapters live in `packages/source-adapters/src/platforms/registry.ts`. The implemented adapter set presently includes `codex`, `claude_code`, `factory_droid`, `amp`, `cursor`, `antigravity`, `gemini`, `openclaw`, `opencode`, `lobechat`, `codebuddy`, and `accio`.
...
- `apps/cli` is the admin / AI-agent surface. It handles data management workflows: sync, discover, health, export/import/backup, GC, remote agent ops, and scriptable `query` with `--json`. Browse commands (ls/tree/show/search/stats) exist for admin inspection but are not the primary end-user experience. Review `apps/cli/src/index.ts` before changing operator workflows.
- `apps/tui` is the primary end-user terminal surface. It should provide a complete read-side experience: browse projects/sessions/turns, drill into full conversations (user prompts + assistant replies + tool calls), search, and view stats — like a file manager for AI coding history. Review `apps/tui/src/index.ts` before changing keyboard-first local browse/search workflows. See `docs/design/UX_IMPROVEMENT_PLAN.md` for the current UX improvement roadmap.
- `apps/web` is the primary end-user web surface. Same scope as TUI but richer: charts, filters, inbox triage, mouse-first interaction.
