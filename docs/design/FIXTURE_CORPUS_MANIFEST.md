...
# Source Families
The current fixture set covers all 14 registered adapters across `local_coding_agent`, `conversational_export`, and `local_runtime_sessions` families.
...
| `Kimi Code` | main-agent `sessions/**/agents/main/wire.jsonl` plus per-session `state.json` | duplicate context echoes are evidence-only; background steers are injected content; user-history and subagent wires remain companion evidence | `platforms/kimi.test.ts` |
