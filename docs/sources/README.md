...
截至 2026-07-18，CCHistory registry 中有 14 个 source adapter。其中 10 个达到 `stable` 自托管支持分级，本目录覆盖的就是这 10 个已通过真实数据验证的 source reference；其余 4 个仍为 `experimental`。
...
5. 结果写入本地 SQLite。常规 CLI `sync` 当前保留 evidence-derived
   raw records、blob metadata、checksums 和 lineage references；byte-exact
   原始文件快照不保证写入所选 store 的 `raw/` 目录。API persisted sync 与
   bundle/export/import 工作流会 materialize raw snapshots。
...
CCHistory 的本地存储由一个 SQLite 数据库、可选的一组按 blob id 命名的
原始快照、以及少量 inspection 产物组成。默认 store 是
`~/.cchistory/`；显式 `--store` / `--db` 只改变落点，不改变内部布局。
典型目录如下：
...
- `raw/<source-id>/` 存的是 API persisted sync、bundle/export/import 等
  工作流 materialize 的原始文件快照；常规 CLI `sync` 不保证该目录存在或
  覆盖所有 blobs。文件名以 `blob.id` 为主，扩展名尽量保留原文件类型。
...
| `kimi` | 需手动确认/覆盖 | 仍属 `experimental`，不要把 Windows 自动发现当成稳定能力。 |

需要手动配置时，请在 Web 的 `Sources` 页面新增/覆盖 `base_dir`，或调用 `/api/admin/source-config`。

# 当前 source 一览

本目录列出已通过真实数据验证、达到 `stable` 分级的 10 个源；`lobechat`、`accio`、`zcode` 和 `kimi` 暂不在此展开。
...
- 它们虽然已经进入 registry / fixture / parser 范围，但仍然属于 `experimental`，还不适合写成"面向开发者的技术说明"并作为稳定参考。
