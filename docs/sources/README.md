...
截至 2026-03-27，CCHistory registry 中有 10 个 source adapter。其中 6 个达到 `stable` 自托管支持分级，本目录覆盖的就是这 6 个已通过真实数据验证的 source reference；其余 4 个仍为 `experimental`。
...
| `gemini` | 需手动确认/覆盖 | 仍属 `experimental`；当前代码会探测 `%USERPROFILE%\.gemini`，但不要把 Windows 自动发现当成稳定能力。 |
| `openclaw` | 需手动确认/覆盖 | 仍属 `experimental`，不要把 Windows 自动发现当成稳定能力。 |
| `opencode` | 需手动确认/覆盖 | 仍属 `experimental`，不要把 Windows 自动发现当成稳定能力。 |
...
本目录列出已通过真实数据验证、达到 `stable` 分级的 6 个源；`gemini`、`openclaw`、`opencode`、`lobechat` 暂不在此展开。
