...
截至 2026-03-20，CCHistory registry 中有 9 个 source adapter。其中 6 个达到 `stable` 自托管支持分级，本目录覆盖的就是这 6 个已通过真实数据验证的 source reference；其余 3 个仍为 `experimental`。
...
本目录列出已通过真实数据验证、达到 `stable` 分级的 6 个源；`openclaw`、`opencode`、`lobechat` 暂不在此展开。
