...
>
> Windows 下当前代码仍会按用户 home 目录探测 `%USERPROFILE%\.codex\sessions`，但这一路径尚未在真实 Windows 主机上完成验证；运维上应在 `Sources` 中确认或覆盖 `base_dir`。
...
- `event_msg.payload.type=token_count` 会变成 token usage signal；当 Codex 同时写入累计值和本次请求值时，解析器按累计基线隔离交错的响应分支，并只保留每个分支的新增用量。
