# Cursor
**结论：Cursor 当前主要通过 VS Code 风格的 `state.vscdb` 和可选的 `agent-transcripts/*.jsonl` 接入；它更像在读编辑器状态库，而不是纯聊天日志。**
...
# 获取方式
**结论：Cursor 的接入不是只扫一种文件，而是把 transcript、workspaceStorage、globalStorage 这几层一起看。**
...
# 上游存储结构
**结论：Cursor 的关键目录通常是 `workspaceStorage/<opaque-id>/`，这里的 `<opaque-id>` 不是项目名，而是编辑器内部 storage id。**
...
# 文件结构
**结论：Cursor 的主数据并不在一个易读的 JSON 文件里，而是在 SQLite key-value store 里。**
...
# CCHistory 当前怎么解释
**结论：CCHistory 优先尝试从 composer/bubble 恢复真正会话；如果恢复不了，就退回 prompt history。**
...
# 注意事项
**结论：Cursor 的难点是“编辑器存储 id”和“用户看到的项目路径”经常不是一回事。**
