...
test("usage errors for incomplete commands happen before store or remote access", async () => {
  ...
  try {
    const cases: Array<{ argv: string[]; pattern: RegExp }> = [
      ...
      { argv: ["query"], pattern: /Use `query turns\|turn\|sessions\|session\|projects\|project\|ask-user \.\.\.`\./ },
      { argv: ["query", "turns", "extra"], pattern: /Use `query turns\|turn\|sessions\|session\|projects\|project\|ask-user \.\.\.`\./ },
