...
const commandSpecs: CommandSpec[] = [
  ...
  },
  {
    ...
    usage: "cchistory query turns|turn|sessions|session|projects|project|ask-user ...",
    summary: "Scriptable JSON-only interface",
    description: "Structured JSON output for programmatic consumption.",
    children: ["turns", "turn", "sessions", "session", "projects", "project", "ask-user"],
    options: readOptions,
    examples: ["cchistory query turns --search refactor --limit 5", "cchistory query turn --id <turn-id>", "cchistory query ask-user --source codex --json"],
  },
  ...
  { path: ["query", "ask-user"], usage: "cchistory query ask-user [--source <slot-or-id>] [--session <session-ref>] [--limit <n>]", description: "Query synthesized AskUserQuestion / request_user_input turns.", options: ["source", "session", "limit", ...readOptions], optionDefaults: { limit: 200 }, examples: ["cchistory query ask-user", "cchistory query ask-user --source claude-code", "cchistory query ask-user --session <session-ref> --json"] },
