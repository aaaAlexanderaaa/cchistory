type OpenApiPaths = Record<string, Record<string, { summary: string }>>;

export function buildOpenApiDocument(options: { includeAgentExtension?: boolean } = {}) {
  const paths: OpenApiPaths = {
    ...
    "/api/turns/summary": { get: { summary: "Summarize turn counts by link state" } },
    ...
    "/api/sessions": { get: { summary: "List resolved sessions" } },
    "/api/sessions/{sessionId}": { get: { summary: "Get session detail" } },
    "/api/admin/sessions/{sessionId}/related-work": {
      get: { summary: "Get typed related-work relations for a session" },
    },
    ...
    "/api/admin/lifecycle/candidate-gc": {
      post: { summary: "Archive or purge candidate turns older than a cutoff" },
    },
    ...
    "/api/admin/source-config/{sourceId}": {
      post: { summary: "Override the base directory for a configured source" },
    },
    "/api/admin/source-config/{sourceId}/reset": {
      post: { summary: "Reset a source base directory back to its default" },
    },
    ...
    "/api/tombstones/{logicalId}": {
      get: { summary: "Resolve a purged logical id to a tombstone projection" },
    },
  };

  if (options.includeAgentExtension) {
    Object.assign(paths, buildAgentExtensionPaths());
  }
...
function buildAgentExtensionPaths(): OpenApiPaths {
  return {
    ...
    "/api/agent/jobs/{jobId}/complete": {
      post: { summary: "Report remote-agent collection job completion or failure" },
    },
    ...
    "/api/admin/agents/{agentId}/labels": {
      post: { summary: "Update operator-facing remote-agent labels and display name" },
    },
  };
}
